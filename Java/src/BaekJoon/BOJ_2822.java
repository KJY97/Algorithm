package BaekJoon;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStreamReader;
import java.util.Arrays;

public class BOJ_2822 {

	/**
	 * 문제
	 *  상근이는 퀴즈쇼의 PD이다. 이 퀴즈쇼의 참가자는 총 8개 문제를 푼다. 
	 *  참가자는 각 문제를 풀고, 그 문제를 풀었을 때 얻는 점수는 문제를 풀기 시작한 시간부터 경과한 시간과 난이도로 결정한다.
	 *  문제를 풀지 못한 경우에는 0점을 받는다. 참가자의 총 점수는 가장 높은 점수 5개의 합이다. 
	 *  상근이는 잠시 여자친구와 전화 통화를 하느라 참가자의 점수를 계산하지 않고 있었다.
	 *  참가자의 8개 문제 점수가 주어졌을 때, 총 점수를 구하라.
	 *  첫째 줄에 참가자의 총점을 출력한다. 
	 *  둘째 줄에는 어떤 문제가 최종 점수에 포함되는지를 공백으로 구분하여 출력한다. 출력은 문제 번호가 증가하는 순서이어야 한다.
	 *   - 점수는 0보다 크거나 같고, 150보다 작거나 같다. 
	 *   - 모든 문제에 대한 점수는 서로 다르다.
	 *  
	 * 시간: 80 ms
	 * 메모리: 11488 kb
	 */
	
	private static class Point implements Comparable<Point> {
		int idx, score; // 문제 번호, 점수

		public Point(int idx, int score) {
			this.idx = idx;
			this.score = score;
		}

		@Override
		public int compareTo(Point o) { 
			// 내림차순 정렬
			return o.score - this.score;
		}
	}
	
	public static void main(String[] args) throws IOException {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		
		// 참가자의 점수 입력
		Point[] person = new Point[8];
		
		for (int i = 0; i < 8; i++) {
			person[i] = new Point(i+1, Integer.parseInt(br.readLine())); // 문제 번호, 점수 
		}
		
		Arrays.sort(person);
		
		int sum = 0; // 참가자의 최종 점수
		int[] result = new int[5]; // 최종 점수에 포함된 문제들
		
		for (int i = 0; i < 5; i++) {
			sum += person[i].score;
			result[i] = person[i].idx;
		}
		
		System.out.println(sum);
		
		// 오름차순 정렬
		Arrays.sort(result);
		
		for (int i = 0; i < 5; i++) {
			System.out.print(result[i] + " ");
		}
	}

}
